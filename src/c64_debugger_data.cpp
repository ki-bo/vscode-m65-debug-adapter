...
C64DebuggerData::C64DebuggerData(const std::filesystem::path& dbg_file)
{
  ...
  throw_if<std::runtime_error>(!root || std::string(root->Name()) != "C64debugger" || !root->Attribute("version", "1.0"),
...
auto C64DebuggerData::get_block_entry(int addr, 
                                      std::string* segment, 
                                      std::string* block) const -> const BlockEntry*
{
...
auto C64DebuggerData::get_file(int idx) const -> std::string
{
    ...
    for (const auto& block : seg.blocks) {
      for (const auto& entry : block.entries) {
        if (entry.file_index != file_index) 
          continue;
        if (line >= entry.line1 && line <= entry.line2) 
          return &entry;
      }
    }
  }
...
{
  throw_if<std::runtime_error>(!block_element->Attribute("name"),
                               "Unsupported dbg Block format");
  ...
  while(std::getline(block_sstr, line))
  {
    ...
    result.entries.emplace_back(BlockEntry {
      .start      = start,
      .end        = end,
